    ...
    connection_params=StdioConnectionParams(
        server_params=StdioServerParameters(
            ...
            args=[
                ...
                settings.filesystem_allowed_dirs[0],
            ],
        ),
        timeout=10.0,
    ),
...
허용 디렉터리:
{','.join(settings.filesystem_allowed_dir)}
...
def _make_notion_search_agent() -> LlmAgent:
    return LlmAgent(
...
def _make_filesystem_search_agent() -> LlmAgent:
    return LlmAgent(
...
def _make_merge_agent() -> LlmAgent:
    return LlmAgent(
        name="MergeAgent",
        model=settings.model,
...
def _make_save_to_file_agent() -> LlmAgent:
    return LlmAgent(
...
def _make_summary_only_agent() -> LlmAgent:
    return LlmAgent(
...
parallel_collect_agent = ParallelAgent(
    ...
    sub_agents=[_make_notion_search_agent(), _make_filesystem_search_agent()],
...
run_sequential_pipeline = SequentialAgent(
    ...
    sub_agents=[
        _make_notion_search_agent(),
        _make_merge_agent(),
        _make_save_to_file_agent(),
    ],
